...
# 启动后端 API 服务
echo "Starting 装歌盘搜 API service on port 9999..."
ENV=production PORT=9999 GOPROXY=https://goproxy.cn,direct ENABLED_PLUGINS="ahhhhfs,aikanzy,alupan,ash,bixin,cldi,clmao,clxiong,cyg,daishudj,ddys,discourse,djgou,duoduo,dyyj,erxiao,feikuai,fox4k,gying,haisou,hdmoli,hdr4k,huban,hunhepan,javdb,jikepan,jsnoteclub,jutoushe,kkmao,kkv,labi,leijing,libvio,lou1,meitizy,miaoso,mikuclub,mizixing,muou,nsgame,nyaa,ouge,pan666,pansearch,panta,panwiki,panyq,pianku,qingying,qqpd,quark4k,quarksoo,qupanshe,qupansou,sdso,shandian,sousou,susu,thepiratebay,u3c3,wanou,weibo,wuji,xb6v,xdpan,xdyh,xiaoji,xiaozhang,xinjuc,xuexizhinan,xys,yiove,ypfxw,yuhuage,yunsou,zhizhen,zxzj" nohup ./pansou > /tmp/pansou.log 2>&1 &
sleep 3

# 检查后端是否启动成功
if ! ss -tuln 2>/dev/null | grep -q ':9999.*LISTEN'; then
  echo "ERROR: Failed to start 装歌盘搜 API service"
  tail -20 /tmp/pansou.log
  exit 1
fi

echo "✅ 装歌盘搜 API started on port 9999"
